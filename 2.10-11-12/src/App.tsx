...
const MovieListPage = () => (
  <div>
    <h1>Movies List</h1>
    <h2>Movies:</h2>
    {defaultFilms.map((movie) =>(
      <Link key={movie.title} to={`/movies/${movie.title}`} style={{display: "block"}}>
        {movie.title}
      </Link>
    ))}
  </div>
...
const MoviePage = () => {
  ...
  const movie = defaultFilms.find((movie) => movie.title === movieTitle)
...
export default App;
export {HomePage, CinemaPage, MovieListPage, MoviePage};